...
pub fn create_deploy(base_url: Option<&str>, config_path: String, trekker: &str, stage: &str, branch: &str, config: Config) -> Result<CreateDeployResponse, Error> {
    let config = try!(config.validate(base_url, config_path));
